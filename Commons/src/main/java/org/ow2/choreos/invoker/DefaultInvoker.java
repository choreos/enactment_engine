...
/**
 * Invokes an external system with a given amount of trials,
 * a timeout for each trial, and a pause between trials. 
 * 
 * This invoker implementation also stores an invocation history
 * if SAVE_HISTORY is set to true.
 * 
 */
class DefaultInvoker<T> implements Invoker<T> {

    public static boolean SAVE_HISTORY = false;

    private static Logger logger = Logger.getLogger(DefaultInvoker.class);
    ...
    DefaultInvoker(String taskName, Callable<T> task, int trials, int trialTimeout, int pauseBetweenTrials, TimeUnit timeUnit) {
