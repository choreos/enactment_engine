...
public class ModelsForTest {
	...
	private void initAirlineSpec() {
		if (packageType == PackageType.COMMAND_LINE) {
			airlineSpec = new DeployableServiceSpec(AIRLINE, serviceType,
					packageType, resourceImpact, serviceVersion, AIRLINE_JAR,
					AIRLINE_PORT, AIRLINE, numberOfAirlineServices);
		} else {
			airlineSpec = new DeployableServiceSpec(AIRLINE, serviceType,
					packageType, resourceImpact, serviceVersion, AIRLINE_WAR,
					AIRLINE, numberOfAirlineServices);
		}
	...
	private void initTravelSpec() {
		if (packageType == PackageType.COMMAND_LINE) {
			travelSpec = new DeployableServiceSpec(TRAVEL_AGENCY, serviceType,
					packageType, resourceImpact, serviceVersion,
					TRAVEL_AGENCY_JAR, TRAVEL_AGENCY_PORT, TRAVEL_AGENCY,
					numberOfTravelInstances);
		} else {
			travelSpec = new DeployableServiceSpec(TRAVEL_AGENCY, serviceType,
					packageType, resourceImpact, serviceVersion,
					TRAVEL_AGENCY_WAR, TRAVEL_AGENCY, numberOfTravelInstances);
		}
