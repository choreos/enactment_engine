...
public class GlimpseProbe extends GlimpseAbstractProbe {
	...
	public GlimpseProbe(String gh) {
		super(Manager.createProbeSettingsPropertiesObject(
				"org.apache.activemq.jndi.ActiveMQInitialContextFactory", gh,
				"system", "manager", "TopicCF", "jms.probeTopic", false,
				"probeName", "probeTopic"));
	}

	public void reportQoSMetric(String metricName, String metricValue,
			String service, String ip) {

		boolean isException = false;
		GlimpseBaseEventChoreos<String> event = new GlimpseBaseEventChoreos<String>(
				metricValue, new Date().getTime(), metricName, isException, "",
				service, ip);
		try {
