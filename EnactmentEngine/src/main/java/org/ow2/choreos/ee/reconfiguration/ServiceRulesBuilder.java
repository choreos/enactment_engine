...
public class ServiceRulesBuilder {

	private Logger logger = Logger.getLogger("reconfLogger");

	private static final String SERVICE_RULE_TEMPLATE = "rules/service_rules_template";
	...
	public String assemblyRules(List<DeployableService> deployableServices,
			String chorID) {
	...
	private String assemblyServiceRule(DeployableService service, String chorID) {
		...
		try {
			serviceRuleTemplate = serviceRuleTemplate.append(FileUtils
					.readFileToString(new File(getClass().getClassLoader()
							.getResource(SERVICE_RULE_TEMPLATE).getFile())));
		} catch (IOException e) {
		...
		if (serviceDesiredQoS == null)
			return "";
		ResponseTimeMetric responseTimeMetric = serviceDesiredQoS
				.getResponseTimeMetric();
		...
		for (ServiceInstance instance : service.getInstances()) {
			...
			String instanceRule = tmp
					...
					.replace(MAX_RESPONSE_TIME_PLACEHOLDER,
							"" + responseTimeMetric.getMaxDesiredResponseTime())
					;
